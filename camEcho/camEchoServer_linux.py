...
class camServer(object):
    def __init__(self, parent, videoSrc=0):
        ...
        self.commThread = echoCamThread(self, 0, "server thread", self.paramDict)
        self.echoframe= None
...
#-----------------------------------------------------------------------------
    def loadConfig(self):
        ...
        paramDict = {   'IPADD': '127.0.0.1', # IPaddress
                        ...
                        'SILAT': 0      # ms simulation the network latency 
        }
...
#-----------------------------------------------------------------------------
    def run(self):
        ...
        while not self.termiate:
            ...
            self.commThread.setData(self.data)

            # Tempory disabled the detection.
            #self.detectTgt(frame)
            if not (self.echoframe is None): 
                cv2.imshow('Echo Back Image',self.echoframe)
            # if q entered whole process will stop 
            if cv2.waitKey(1) == ord('q'):
                self.termiate = True 
            time.sleep(1.0/self.paramDict['FRATE'])

        # Destroying all the windows when user quit.        
        cv2.destroyAllWindows() 


    def setEchoFrame(self, frame): 
        self.echoframe = frame
...
class echoCamThread(threading.Thread):
    ...
    def run(self):
        while not self.termiate:
            ...
            time.sleep( 1.0/self.paramDict['FRATE'] +float(self.paramDict['SILAT'])/1000)
            if self.data:
                imgSz = len(self.data)
                ...
                self.data = None # reset the data to null after sent. 
                #if cv2.waitKey(1) == ord('q'):
                #    self.termiate = True 
    ...
    def showEchoImage(self, frame, imgSz, rcvIterN):
        ...
        Imglatency = timeInterval*1000/2 + int(self.paramDict['SILAT'])
        #print(">> %s" %str(self.paramDict['SILAT']) )
        ...
        #print("xxxx")
        #cv2.imshow('Echo Back Image',frame)
        self.parent.setEchoFrame(frame)
