...
#-----------------------------------------------------------------------------
    def loadConfig(self):
        ...
        paramDict = {   'IPADD': '127.0.0.1', # VPN IPaddress
                        'PUBIP': '127.0.0.1', # PUB IPaddress
        ...
        with open(CONFIG_FILE, "r") as fh:
            ...
            for line in lines:
                ...
                key, val = line.split(':')
                paramDict[key] = int(val) if (key != 'IPADD' and key != 'PUBIP')else val
        return paramDict
...
#-----------------------------------------------------------------------------
    def run(self):
        ...
        while not self.termiate:
            # Send one message throught public IP
            time1 = time.time()
            newMsg = self.client.sendMsg("msg", resp=True, ipAddr=(self.paramDict['PUBIP'],UDP_PORT) )
            #print(newMsg)
            time2 = time.time() - time1 + 0.0000000001
            ...
            for _ in range(int(rcvIterN)):
                ...
                if len(self.data) > BUFFER_SZ:
                    ...
                    msg = self.data
                imgData += self.client.sendMsg(msg, resp=True, ipAddr=(self.paramDict['IPADD'], UDP_PORT))
            ...
            self.showEchoImage(frame, imgSz, rcvIterN, time2)
...
#-----------------------------------------------------------------------------
    def showEchoImage(self, frame, imgSz, rcvIterN, timeStr):
        ...
        cv2.putText(frame,"VPN ImgLatency[ms]: %s" %str(Imglatency) , (30,yPos+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), thickness = 2)
        cv2.putText(frame,"VPN DataLatency[ms]: %s" % str(datalatency) , (30 ,yPos+50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), thickness = 2)
        cv2.putText(frame,"No VPN DataLatency[ms]: %s" % str(timeStr) , (30 ,yPos+80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), thickness = 2)
        cv2.putText(frame,"VPN DataRate[kbps]: %s" % str(dataRate) , (30 ,yPos+110), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), thickness = 2)
